...
public class Rocket {
    ...
    public static Rocket fromString(String s) throws Exception {
        try {
            ...
            for (String effect : e) {
                ...
                meta.addEffect(FireworkEffect.builder().flicker(Boolean.parseBoolean(d[0])).trail(Boolean.parseBoolean(d[1]))
                        .with(Type.valueOf(d[2])).withColor(colors).withFade(fadeColors).build());
            }
            return new Rocket(meta, true);
        } catch (Exception e) {
    ...
    public static List<FireworkEffect> randomizeEffects(int min, int max, int minColors, int maxColors) {
        ...
        for (int a = 1; a <= calculateRandom(min, max); a++)
            effects.add(FireworkEffect.builder().flicker(RANDOM.nextBoolean())
                    .with(Type.values()[RANDOM.nextInt(Type.values().length)]).trail(RANDOM.nextBoolean())
